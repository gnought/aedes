...
const { Writable, finished } = require('readable-stream')
...
function Client (broker, conn, req) {
  ...
  const mqttStream = new Writable({ objectMode: true, emitClose: false, encoding: 'utf8' })
  mqttStream._write = function (packet, encoding, done) {
    handle(that, packet, done)
  }
  mqttStream.on('error', this.emit.bind(this, 'error'))
