...
const _noopStream = new Writable({
...
function authenticate (arg, done) {
  ...
  function negate (err, successful) {
    ...
    // remove LWT when authentication fails
    client._will = null
    if (err) {
...
function authorizeConnect (arg, done) {
  const client = this.client
  if (typeof client.broker.authorize !== 'function') {
    return done()
  }
  ...
  client.broker.authorizeConnect(client, p, doAuthorize)

  function doAuthorize () {
    packet.keepalive = p.keepalive
    client.clean = packet.clean = p.clean
    client._will = packet.will = p.will
    // validate packet
    _noopStream.on('error', function (err) {
      client.close(done.bind(this, err))
    })
    // console.log(packet)
    const result = mqtt.writeToStream(packet, _noopStream)
